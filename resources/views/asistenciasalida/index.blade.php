                  ...
                  <tr>
                    ...
                    <td>
                      <form action="{{ route('asistenciasalidas.destroy', $asistenciasalida->id) }}" method="POST">
                        ...
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('¿Esta seguro que desea eliminar este registro?')" class="btn btn-danger btn-sm" onclick="event.preventDefault(); confirm('Are you sure to delete?') ? this.closest('form').submit() : false;"><i class="fa fa-fw fa-trash"></i> {{ __('Delete') }}</button>
                      </form>
                    </td>
                  </tr>
                  @endforeach
  ...
  $(function() {
    $("#example1").DataTable({
      ...
      "language": {
        ...
        "info": "Mostrando _START_ a _END_ de _TOTAL_ Asistencias de Salida",
        "infoEmpty": "Mostrando 0 a 0 de 0 Asistencias de Salida",
        "infoFiltered": "(Filtrado de _MAX_ total Asistencias de Salida)",
        "infoPostFix": "",
        "thousands": ",",
        "lengthMenu": "Mostrar _MENU_ Asistencias de Salida",
